...
const totalUnfundedGames = GAMES_JSON.filter( (games) => {
    return games.pledged < games.goal;
});
let NoOfUnfundedGames = totalUnfundedGames.length;

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of ${totalPledged.toLocaleString('en-US')} has been raised for 
                    ${totalGames.toLocaleString('en-US')} games. Currently, 
                    ${NoOfUnfundedGames.toLocaleString('en-US')} 
                    ${NoOfUnfundedGames == 1 ? "game remains" : "games remain"} unfunded.
                    We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
const infoStr = document.createElement("p");
infoStr.textContent = displayStr;
descriptionContainer.appendChild(infoStr);
